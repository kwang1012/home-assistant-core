...
from homeassistant.const import ATTR_DEVICE_CLASS, ATTR_EDITABLE, CONF_ID
...
async def async_setup_entry(
    ...
    for entity_config in get_entity_configs(
        ...
        if entity_config[CONF_CLASS] in (
            VirtualTimerDeviceClass.COFFEE_MACHINE,
            VirtualTimerDeviceClass.DISHWASHER,
            VirtualTimerDeviceClass.DRYER,
            VirtualTimerDeviceClass.MICROWAVE,
            VirtualTimerDeviceClass.MOWER,
            VirtualTimerDeviceClass.OVEN,
            VirtualTimerDeviceClass.SPRINKLER,
            VirtualTimerDeviceClass.TOASTER,
            VirtualTimerDeviceClass.WASHER,
        ):
            component = importlib.import_module(
                f"homeassistant.components.virtual.{entity_config[CONF_CLASS]}"
            )
            entity = await component.async_setup_entity(
                hass, entity_config, coordinator
            )
...
class VirtualTimer(Timer, VirtualEntity):
    ...
    def __init__(self, config) -> None:
        ...
        self._attr_device_class = config.get(
            CONF_CLASS, VirtualTimerDeviceClass.COFFEE_MACHINE
        )

        self._change_time: timedelta = config.get(CONF_CHANGE_TIME)
        self._duration: timedelta = timedelta()
    ...
    def _restore_state(self, state, config):
        ...
        if config.get(CONF_RESTORE, DEFAULT_RESTORE):
            ...
            self._remaining = cv.time_period(
                state.extra_state_attributes[ATTR_REMAINING]
            )
    ...
    def _update_attributes(self):
        ...
        self._attr_is_remaining = (
            self._remaining.total_seconds() > 0
            if self._remaining is not None
            else False
        )
        if self._restore:
