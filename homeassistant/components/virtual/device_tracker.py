...
async def async_setup_entry(
    ...
    """Set up virtual device tracker entries."""
...
class VirtualDeviceTracker(TrackerEntity, VirtualEntity):
    ...
    def __init__(self, config):
        ...
        _LOGGER.debug("%s, available=%s", self._attr_name, self._attr_available)
        _LOGGER.debug("%s, entity=%s", self._attr_name, self.entity_id)
    ...
    def move(self, new_location):
        ...
        _LOGGER.debug("%s moving to %s", self._attr_name, new_location)
        self._location = new_location
        self.async_schedule_update_ha_state()


async def async_virtual_move_service(hass, call):
    """Handle the virtual move service call."""
    for entity_id in call.data["entity_id"]:
        _LOGGER.debug("moving %s", entity_id)
