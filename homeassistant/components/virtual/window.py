...
async def async_setup_entry(
    ...
    for entity_config in get_entity_configs(
        ...
        if entity_config[CONF_COORDINATED]:
            entity = cast(
                VirtualWindow, CoordinatedVirtualWindow(entity_config, coordinator)
            )
        else:
            entity = VirtualWindow(entity_config)

        if entity_config[CONF_SIMULATE_NETWORK]:
            entity = cast(VirtualWindow, NetworkProxy(entity))
...
class VirtualWindow(VirtualEntity, CoverEntity):  # pylint: disable=hass-enforce-class-module
    ...
    def __init__(self, config):
        ...
        if self._attr_device_class == CoverDeviceClass.WINDOW:
            self._dataset = load_dataset(Dataset.WINDOW)
...
class CoordinatedVirtualWindow(CoordinatedVirtualEntity, VirtualWindow):  # pylint: disable=hass-enforce-class-module
    """Representation of a Virtual switch."""

    def __init__(self, config, coordinator):
        """Initialize the Virtual switch device."""
        CoordinatedVirtualEntity.__init__(self, coordinator)
        VirtualWindow.__init__(self, config)
