...
PLATFORM_SCHEMA = LIGHT_PLATFORM_SCHEMA.extend(BASE_SCHEMA)
...
class VirtualLight(VirtualEntity, LightEntity):
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        if (
            ...
            and ColorMode.COLOR_TEMP in self._attr_supported_color_modes  # type: ignore[operator]
        ):
            self._attr_color_mode = ColorMode.HS
        ...
        if ct is not None and ColorMode.COLOR_TEMP in self._attr_supported_color_modes:  # type: ignore[operator]
            self._attr_color_mode = ColorMode.COLOR_TEMP
