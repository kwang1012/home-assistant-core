...
class RaspberryPiDoor(RaspberryPiDevice):
    ...
    async def open_door(self, transition: int | None = None):
        """Open the door."""
        _state = {"on_off": 1}
        ...
        return await self._query_helper(self.DOOR_SERVICE, self.SET_DOOR_METHOD, _state)

    async def close_door(self, transition: int | None = None):
        """Close the door."""
        _state = {"on_off": 0}
