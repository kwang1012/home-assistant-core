...
class RaspberryPiDevice:
    ...
    def __init__(self, addr, *args, port=None, **kwargs) -> None:
        """Initialize device."""
        self.host, self.port = self._parse_hostport(
            addr, port or RaspberryPiDevice.DEFAULT_PORT
        )
        self.addr = addr
        self.reader: asyncio.StreamReader | None = None
        self.writer: asyncio.StreamWriter | None = None
        self.query_lock: asyncio.Lock | None = None
    ...
    def is_fan(self) -> bool:
        """Return true if device is fan."""
        return self.device_type == "fan"

    @property
    def is_lock(self) -> bool:
        """Return true if device is lock."""
        return self.device_type == "lock"

    @property
    def is_light(self) -> bool:
        """Return true if device is light."""
        return self.device_type == "light"

    @property
    def is_switch(self) -> bool:
        """Return true if device is switch."""
    ...
    async def query(self, request: str | dict, retry_count: int = 3):
    ...
    def _create_request(self, target: str, cmd: str, arg: dict | None = None):
        request: dict[str, Any] = {target: {cmd: arg}}

        return request

    async def _query_helper(
        self, target: str, cmd: str, arg: dict | None = None
    ) -> Any:
    ...
    def _parse_hostport(self, addr: str, default_port):
        if addr[-1] == "]":
        ...
        if len(out) == 1:
            ...
            try:
                ...
                raise ValueError(f"Invalid host:port '{addr}'") from None
