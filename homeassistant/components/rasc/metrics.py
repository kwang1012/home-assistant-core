...
class ScheduleMetrics:
    ...
    def __init__(
        ...
        if rescheduling_policy:
            ...
            self._rescheduling_policy = sm._rescheduling_policy  # noqa: SLF001
            self._result_dir = sm._result_dir  # noqa: SLF001
            self._version = sm._version  # noqa: SLF001
            self._arrival_times = copy.deepcopy(sm._arrival_times)  # noqa: SLF001
            self._remaining_actions = copy.deepcopy(sm._remaining_actions)  # noqa: SLF001
            self._start_times = copy.deepcopy(sm._start_times)  # noqa: SLF001
            self._end_times = copy.deepcopy(sm._end_times)  # noqa: SLF001
            self._wait_times = copy.deepcopy(sm._wait_times)  # noqa: SLF001
            self._exec_times = copy.deepcopy(sm._exec_times)  # noqa: SLF001
            self._first_arrival_time = sm._first_arrival_time  # noqa: SLF001
            self._schedule_start = sm._schedule_start  # noqa: SLF001
            self._schedule_end = sm._schedule_end  # noqa: SLF001
            self._action_times = copy.deepcopy(sm._action_times)  # noqa: SLF001
            self._last_action_end = copy.deepcopy(sm._last_action_end)  # noqa: SLF001
            self._idle_times = copy.deepcopy(sm._idle_times)  # noqa: SLF001
            self._parallelism = copy.deepcopy(sm._parallelism)  # noqa: SLF001
    ...
    def _remove_routine_remaining_action(self, action_id: str, entity_id: str) -> None:
        ...
        # LOGGER.debug("[remaining] Action %s completed on entity %s", action_id, entity_id)
    ...
    def schedule_length(self) -> timedelta:
        ...
        if not self._schedule_start:
            raise ValueError(f"Schedule has not started: {self}")
        if not self._schedule_end:
            raise ValueError(f"Schedule has not ended: {self}")
    ...
    def avg_parallelism(self) -> float:
        ...
        for time, parallelism in parallelisms.items():
            ...
            weighted_parallelism_sum += timediff * last_parallelism  # type: ignore[assignment]
            last_time = time
            last_parallelism = parallelism
