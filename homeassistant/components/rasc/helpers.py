...
from .const import CONF_ENABLED, RASC_USE_UNIFORM
...
class OverheadMeasurement:
    ...
    def _handle_reschedule_event(self, event: Event):
        ...
        self._reschedule_intervals.append((start_time, end_time, diff))  # type: ignore[arg-type]
    ...
    async def _start(self) -> None:
        while not self._terminate_flag.is_set():
            ...
            curr, _peak = tracemalloc.get_traced_memory()
            ...
            json.dump(
                {
                    "cpu": self._cpu_usage,
                    "mem": self._mem_usage,
                    "mem_in_bytes": self._mem_in_bytes,
    ...
    def __str__(self) -> str:
        ...
        if self._config.get(RASC_USE_UNIFORM):
            return f"om_static_{self._config[CONF_SCHEDULING_POLICY]}_{self._config[CONF_RESCHEDULING_POLICY]}_{self._config[CONF_RESCHEDULING_TRIGGER]}_{self._config[ACTION_LENGTH_ESTIMATION]}_{filename}"
...
def fire(
    ...
    if logger:
        logger.debug(
            ...
            entity_id,
            action,
            service_data.get(ATTR_ACTION_ID, ""),  # type: ignore[union-attr]
            rasc_type,
        )
    ...
    hass.bus.async_fire(
        RASC_RESPONSE,
        {
            ...
            **{
                str(key): _format_timedelta(value)
