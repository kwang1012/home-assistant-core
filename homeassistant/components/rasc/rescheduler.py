...
class BaseRescheduler(TimeLineScheduler):
    ...
    async def _move_device_schedule(
        ...
        st_time is the time in the schedule after which actions are being moved.
    ...
    async def move_device_schedules(
        ...
        tasks = [
            asyncio.create_task(self._move_device_schedule(entity_id, st_time, diff))
    ...
    def RV(
        ...
        if rescheduled_action_info and rescheduled_action_entity_id:
        ...
        for entity_id, lock_queue in lock_queues.items():
            ...
            for action_id, action_lock in lock_queue.items():
                ...
                if action_id != rescheduled_action_info.action_id or (  # type: ignore[union-attr]
                    action_id == rescheduled_action_info.action_id  # type: ignore[union-attr]
        ...
        if (
            rescheduled_action_info
            and rescheduled_action_entity_id
            and rescheduled_action_st_time
            and rescheduled_action_end_time
        ):
        ...
        while next_action_heap:
            ...
            if action_id in visited:
                continue
            LOGGER.debug("Rescheduling %s", action_id)
            ...
            if action_id == rescheduled_action_info.action_id:  # type: ignore[union-attr]
            ...
            for entity_id in entities:
                if entity_id not in free_slots:
                    raise ValueError(f"Entity {entity_id} has no free slots.")
                if (
                    entity_id == rescheduled_action_entity_id
                    and action_id == rescheduled_action_info.action_id  # type: ignore[union-attr]
                ):
                    continue
                ...
                action_end = action_st + action.length(entity_id)  # type: ignore[operator]
                LOGGER.debug(
                    "Try to schedule action %s to %s-%s on %s",
                    action_id,
                    datetime_to_string(action_st),
                    datetime_to_string(action_end),  # type: ignore[arg-type]
                    entity_id,
                )
                LOGGER.debug(
                    "Lock queue status: %s",
                    [
                        f"{action_info.action_id}: {datetime_to_string(action_info.start_time)}-{datetime_to_string(action_info.end_time)}"  # type: ignore[union-attr]
                ...
                free_slot = self._find_slot_including_time_range(
                    self._lineage_table.free_slots[entity_id],
                    (action_st, action_end),  # type: ignore[arg-type]
                    entity_id,
                )
                self.schedule_action(
                    free_slot,
                ...
                cur_action_lock.move_to(action_st, action_end, entity_id)  # type: ignore[arg-type, union-attr]

                # update the schedule metrics
                rv_metrics.record_scheduled_action_start(
                    action_st,  # type: ignore[arg-type]
                    entity_id,
                    action.action_id,
                )
                rv_metrics.record_scheduled_action_end(
                    action_end,  # type: ignore[arg-type]
                    entity_id,
                    action.action_id,
                )
    ...
    def _is_action_running(
        self, action_id: str, time: datetime = datetime.now(UTC)
    ) -> bool:
    ...
    def _routine_actions_after(
        ...
        if not routine_info:
            raise ValueError(f"Routine {routine_id} has not been scheduled.")
    ...
    def get_immutable_serialization_order(self, now: datetime) -> list[str]:
    ...
    def deschedule_affected_actions(  # pylint: disable=too-many-nested-blocks
        ...
        LOGGER.debug("affected_action_ids=%s", affected_action_ids)
        ...
        while extra_affected_action_ids:
            ...
            for entity_id, lock_queue in self._lineage_table.lock_queues.items():
                ...
                for action_id in actions_to_remove:
                    descheduled_action = lock_queue.pop(action_id).action  # type: ignore[union-attr]
    ...
    def apply_serialization_order_dependencies(
        ...
        printed_entity_descheduled_actions = {}  # type: ignore[var-annotated]
    ...
    def _are_all_parents_scheduled(
        self, action: ActionEntity, lt: LineageTable | None = None
    ) -> bool:
    ...
    def sjf(  # noqa: C901
        ...
        immutable_serialization_order: list[str] | None,
        metrics: ScheduleMetrics,
        # window: timedelta
        ...
        for action in descheduled_actions.values():
            ...
            for target_entity in target_entities:
                ...
                # routine_info = self._serialization_order.get(get_routine_id(action.action_id))
                # heapq.heappush(
                #     wait_queues[entity_id], (routine_info.routine.start_time, action)
                # )
                heapq.heappush(
        ...
        while descheduled_actions:
            ...
            action_st = max(action_st, datetime.now(UTC))
            ...
            for child in shortest_action.all_children:
                ...
                for target_entity in target_entities:
                    ...
                    if entity_id not in wait_queues:
                        wait_queues[entity_id] = list[tuple[timedelta, ActionEntity]]()
                    # routine_info = self._serialization_order.get(get_routine_id(child.action_id))
                    # heapq.heappush(
                    #     wait_queues[entity_id], (routine_info.routine.start_time, child)
                    # )
                    heapq.heappush(
    ...
    def _optimal_helper(
        ...
        prev_serialization_order: Queue[str, RoutineInfo] | None,
        ...
        if serializability_guarantee:
            ...
            if (
                ...
                serialization_order[chosen_action_routine_id] = (
    ...
    def optimal(
        ...
        next_entity_id, _next_slot_st = min(
            filtered_next_slots.items(), key=lambda x: x[1]
        )
        # LOGGER.debug(
        #     "next entity: %s, next slot start: %s",
    ...
    def _identify_first_common_idle_time_after(
        ...
        ) -> tuple[datetime, datetime | None] | tuple[None, None]:
    ...
    def _routine_source_actions(self, routine_id: str) -> list[ActionEntity]:
        ...
        if not routine_info:
            raise ValueError(f"Routine {routine_id} has not been scheduled.")
        routine = routine_info.routine
        return [action for action in routine.actions.values() if not action.all_parents]
    ...
    def _find_actions_breaking_routine_order(
        ...
        for routine_after_id in routines_after:
            routine_actions_to_displace[routine_after_id] = (
            ...
            for parent_action in parent_set:
                ...
                for target_entity in target_entities:
                    ...
                    if parent_action_id not in lock_queues[entity_id]:
                        ...
                        raise ValueError(
                            f"Action {action.action_id}'s parent {parent_action_id} has not been scheduled on entity {entity_id}."
                        )
                    parent_lock = lock_queues[entity_id][parent_action_id]
                    if not parent_lock:
                        raise ValueError(
                            f"Action {parent_action_id}'s schedule information on entity {entity_id} is missing."
                        )
    ...
    def _action_start_time(self, action: ActionEntity) -> datetime:
        ...
        if not target_entities:
            raise ValueError(
                f"Action {action.action_id} does not have any target entities."
            )
        ...
        if not earliest_action_start:
            raise ValueError(f"Action {action.action_id} has not been scheduled.")
    ...
    def _remove_routine_from_serialization_order(self, routine_id: str) -> None:
        ...
        LOGGER.debug("Remove routine %s from the serialization order", routine_id)
    ...
    async def displace_action_to_idle_time(  # unused so far
        ...
        routines_after_actions: dict[str, set[ActionEntity]] = (
            ...
            if set(current_routine_action_set) != set(complete_routine_action_set):
                for action in current_routine_action_set:
                    ...
                    for target_entity_id in target_entities:
                        ...
                        if not action_lock:
                            raise ValueError(
                                f"Action {action.action_id}'s schedule information on entity {target_entity_id} is missing."
                            )
        ...
        if self._routine_prioriy_policy in (EARLIEST, SHORTEST):
            ...
            routine_order = dict.fromkeys(whole_routines_to_reschedule, 0.0)
    ...
    def _eligibility_test(  # noqa: C901
            ...
            for parent_action in parent_set:
                ...
                for parent_entity in parent_target_entities:
                    ...
                    if (
                        ...
                        raise ValueError(
                            f"Parent action {parent_action_id} has not been scheduled on entity {parent_entity_id}."
                        )
                    ...
                    if not parent_action_lock:
                        raise ValueError(
                            f"Parent action {parent_action_id}'s schedule information on entity {parent_entity_id} is missing."
                        )
                    if dependency in (RASC_ACK, RASC_START):
        ...
        for entity_action_id, entity_action_lock in entity_lock_queue.items():
            if not entity_action_lock:
                raise ValueError(
                    f"Action {entity_action_id}'s schedule information on entity {entity_id} is missing."
                )
    ...
    def _find_action_to_fill_slot(
        self, entity_id: str, time_range: tuple[datetime, datetime | None]
    ...
    async def fill_holes(self) -> LineageTable:  # unused so far
        ...
        """
        indices = dict.fromkeys(self._lineage_table.free_slots, 0)
        max_indices = {
            entity_id: len(slots) - 1
            for entity_id, slots in self._lineage_table.free_slots.items()
        }
        while any(indices[entity_id] < max_indices[entity_id] for entity_id in indices):  # pylint: disable=consider-using-dict-items
            chosen_time = None
...
class RascalRescheduler:
    ...
    Attributes:
        ...
        _mis (dict[str, float]): The dictionary of entity IDs and their Mi thresholds.
    """
    ...
    def __init__(
        ...
        self._hass = hass
        self._scheduler = scheduler
        scheduler.set_reschedule_handler(self.handle_event)
        self._resched_policy: str = config[CONF_RESCHEDULING_POLICY]
        scheduler.set_rescheduling_policy(self._resched_policy)
        ...
        self._timer_handles: dict[str, tuple[str | None, Callable[[], None] | None]] = (
            dict.fromkeys(scheduler._lineage_table.lock_queues, (None, None))  # noqa: SLF001
        )
        ...
        if self._estimation:
            ...
            self._mis = dict.fromkeys(scheduler._lineage_table.lock_queues, 0.0)  # noqa: SLF001
        self._reschedule_lock = asyncio.Lock()

    def as_dict(self) -> dict[str, Any]:
        """Return dict representation of the rescheduler."""
        return {
            "resched_policy": self._resched_policy,
            "resched_trigger": self._resched_trigger,
            "optimal_sched_metric": self._optimal_sched_metric,
            "resched_window": self._resched_window,
            "routine_priority": self._routine_priority,
            "estimation": self._estimation,
            "timer_handles": self._timer_handles,
    ...
    async def _move_device_schedules(
        self, time: datetime = datetime.now(UTC), extra: timedelta = timedelta(0)
    ) -> bool:
    ...
    def _reschedule(self, entity_id: str, action_id: str, diff: timedelta) -> None:
        ...
        st_time, old_end_time = action_lock.time_range  # type: ignore[union-attr]
        ...
        if self._resched_policy != RV:
            action_lock.move_to(st_time, new_end_time)  # type: ignore[union-attr]
        ...
        new_lt = None
        new_so = self._rescheduler.serialization_order.clone()
        ...
        if self._resched_policy in (RV):
            try:
                new_lt = self._rescheduler.RV(
                    ...
                    metrics,
                    rescheduled_action=(action_lock, entity_id, st_time, new_end_time),  # type: ignore[arg-type]
                )
            ...
            if not affected_actions:
                LOGGER.debug("No affected source actions found, nothing to reschedule")
            ...
            if serializability:
                self._rescheduler.apply_serialization_order_dependencies(
                    immutable_serialization_order,  # type: ignore[arg-type]
                    descheduled_actions,
                )
            if self._resched_policy in (SJFWO, SJFW):
                ...
                #         if not success:
                #             raise ValueError(
                #                 "Failed to move device schedules.")
    ...
    def _apply_schedule(
        ...
        for entity_id in schedule.lock_queues:
            ...
            for action_id in action_ids:
                ...
                if (
                    action_lock.action.start_requested  # type: ignore[union-attr]
                    != original_action_lock.action.start_requested  # type: ignore[union-attr]
                ):
                    self._hass.data["rasc_events"].append(
                        (
                            datetime.now(UTC).strftime("%H:%M:%S.%f")[:-3],
                    ...
                    action_lock.action.start_requested = True  # type: ignore[union-attr]
    ...
    def _init_timer_delay(self, event: Event) -> timedelta:
        ...
        action = self._scheduler.get_action(action_id)  # type: ignore[arg-type]
        if not action:
        ...
        expected_action_length_sec = action.length(entity_id).total_seconds()  # type: ignore[arg-type]
    ...
    def _get_extra_anticipatory(
        ...
        if action_complete:
            return 0.0
        return expected_action_length.total_seconds() * 0.1
    ...
    def _get_extra_proactive(
        ...
        action_length_estimate = generate_duration(
            rasc.get_action_length_estimate(
                entity_id, action.service, context_id, action.transition
            )
            + rasc.get_action_length_estimate(
                entity_id, action.service, context_id, action.transition, "rts"
            )
        )
        ...
        if action_length_estimate == expected_action_length:
            return 0.0
        return (action_length_estimate - expected_action_length).total_seconds()
    ...
    def _handle_overtime(self, event: Event, diff: float) -> None:
        ...
        entity_id: str | None = event.data.get(ATTR_ENTITY_ID)
        action_id: str | None = event.data.get(ATTR_ACTION_ID)
        context_id: str | None = event.data.get(ATTR_CONTEXT_ID)
        event_type: str | None = event.data.get(CONF_TYPE)
        if not action_id:
            raise ValueError("Action ID is missing in the event.")
        routine_id = get_routine_id(action_id)
        LOGGER.debug("Handling overtime of %s seconds for %s", diff, action_id)
        ...
        if entity_id in self._timer_handles:
            ...
            if saved_action_id != action_id:
                raise ValueError(
                    f"Action ID mismatch for entity {entity_id} in the timer handle. saved: {saved_action_id}, new: {action_id}"
                )
        ...
        if self._resched_trigger in (ANTICIPATORY, REACTIVE):
            ...
            extra = self._get_extra_proactive(
                action,
                entity_id,
    ...
    async def _wait_and_handle_overtime(
        ...
        # try:
        await asyncio.sleep(timer_delay)  # type: ignore[call-overload]
        # except asyncio.CancelledError:
        #     LOGGER.info("CancelledError for %s on %s", action_id, entity_id)
        # return
        self._handle_overtime(event, timer_delay)  # type: ignore[arg-type]
    ...
    def setup_overtime_check(self, event: Event, timer_delay: timedelta) -> None:
        ...
        if isinstance(timer_delay, timedelta):
            timer_delay = timer_delay.total_seconds()  # type: ignore[assignment]
        if timer_delay <= 0:  # type: ignore[operator]
        ...
        LOGGER.debug(
            ...
            entity_id,
            timer_delay,
            datetime.fromtimestamp(t.time(), UTC),
            datetime.now(UTC),
            datetime.fromtimestamp(t.time() + timer_delay, UTC),  # type: ignore[operator]
            # self._timer_handles,
        )
        ...
        self._timer_handles[entity_id] = (action_id, task.cancel)  # type: ignore[assignment, index]
    ...
    def _action_length_diff(self, event: Event) -> timedelta:
        ...
        if event_type == RASC_START:
            exp_time = (
                action_lock.start_time.replace(tzinfo=UTC)
                + action_lock.action.rts[entity_id]
            )
        elif event_type == RASC_COMPLETE:
            exp_time = action_lock.end_time.replace(tzinfo=UTC)
        act_time = event.time_fired.replace(tzinfo=UTC)
        LOGGER.debug(
    ...
    def _handle_undertime(self, event: Event) -> None:
        ...
        LOGGER.debug("Diff: %f", diff.total_seconds())
    ...
    def handle_event(self, event: Event) -> None:
        ...
        if response == SCHEDULE_START:
            ...
            self._handle_overtime(event)  # type: ignore[call-arg]
